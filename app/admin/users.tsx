...
export default function UserManagementScreen() {
  ...
  const handleUpdateAdminStatus = async (newAdminStatus: boolean) => {
    ...
    try {
      ...
      const success = await updateUserAdminStatus(
        selectedUser.id,
        newAdminStatus
      );
      ...
      if (success) {
        ...
        if (updatedUser.is_admin === newAdminStatus) {
          ...
          showAlert(
            "Error",
            "Failed to verify the admin status update. Please try again."
          );
        }
      } else {
      ...
      <ThemedView style={styles.container}>
        ...
        <ThemedView style={styles.section}>
          ...
          {searchResults.length > 0 && (
            <View
              style={[styles.searchResults, isLoading && styles.disabledInput]}
            >
          ...
          {selectedUser && (
            <View
              style={[styles.selectedUser, isLoading && styles.disabledInput]}
            >
              <ThemedText style={styles.selectedUserName}>
                {selectedUser.full_name}
              </ThemedText>
              <ThemedText style={styles.selectedUserEmail}>
                {selectedUser.email}
              </ThemedText>
