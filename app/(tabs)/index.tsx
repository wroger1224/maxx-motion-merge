...
export default function DashboardScreen() {
  ...
  const badgeList = [
    ...
    },
    {
      ...
      emoji: "🏆",
      imageUrl:
        "https://images.unsplash.com/photo-1549060279-7e168fcee0c2?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60",
    },
    {
      id: "7",
      name: "Runner's Badge",
      ...
      emoji: "🏃",
      imageUrl:
        "https://images.unsplash.com/photo-1476480862126-209bfaa8edc8?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60",
    },
    {
      id: "8",
    ...
    },
    {
      ...
      emoji: "🌙",
      imageUrl:
        "https://images.unsplash.com/photo-1517838277536-f5f99be501cd?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60",
    },
  ];
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const totalMinutes =
        userActivities?.reduce((sum, a) => sum + a.activity_minutes, 0) || 0;
      ...
      if (milestones && milestones.length > 0) {
        ...
        for (let i = 0; i < milestones.length; i++) {
          if (totalMinutes < milestones[i].milestone_minutes) {
            currentMilestone =
              prev.milestone_name + ` (${prev.milestone_minutes} min)`;
            nextMilestone =
              milestones[i].milestone_name +
              ` (${milestones[i].milestone_minutes} min)`;
        ...
        if (!found) {
          ...
          currentMilestone =
            milestones[milestones.length - 1].milestone_name +
            ` (${milestones[milestones.length - 1].milestone_minutes} min)`;
      ...
      for (const team of allTeams || []) {
        ...
        const teamMinutes =
          teamActivities?.reduce((sum, a) => sum + a.activity_minutes, 0) || 0;
        leaderboard.push({
          id: team.id,
          ...
          minutesPerMember:
            memberIds.length > 0
              ? Math.round(teamMinutes / memberIds.length)
              : 0,
      ...
      const { data: recentActs } = await supabase
        .from("activities")
        .select(
          "id, activity_type, activity_date, activity_minutes, activity_source"
        )
        .eq("user_id", user.id)
        .eq("event_id", currentEvent.id)
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scrollContent}>
        ...
        <ThemedView style={[styles.section, styles.sectionLast]}>
          ...
          <ThemedText>{renderRecentBadges()}</ThemedText>
        </ThemedView>
      </ScrollView>
    </View>
  );
